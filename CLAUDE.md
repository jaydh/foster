...
| **Dev overlay** | ~~Floating panel injected via `<script>` tag in debug builds.~~ **Done** — `OVERLAY_JS` injected into served template HTML in debug builds. Waits for WASM to stamp `data-fx-session`, then subscribes to SSE for live state/version/last-event updates. Jump-to-state dropdown POSTs to `/test/state` preserving current context. Links to graph and history pages. Minimize state persisted in `sessionStorage`. `Snapshot.last_event: Option<String>` added to foster-core (set by `send()`, `None` for `restore()`). |
...
- Dev overlay: `OVERLAY_JS` injected into served HTML in debug builds — state, version, last event, jump-to-state, graph/history links — `crates/foster-server/src/lib.rs`
