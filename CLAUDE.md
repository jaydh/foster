...
| Item | Design notes |
|------|-------------|
| **HA / multi-replica** | `StateStore` + `PubSub` traits are already defined in `crates/foster-server/src/store.rs`. `AppState` is now generic over both traits; `router_with(machines, store, pubsub)` accepts any `StateStore + Clone` + `PubSub + Clone` implementation. Remaining work: implement `RedisStore` and `RedisPubSub` using `redis-rs`. Use a Redis Lua CAS script or `WATCH`/`MULTI` for atomic [`StateStore::apply`]. The `version` field on `Snapshot` is the optimistic lock token. |
