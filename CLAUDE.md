...
| Item | Design notes |
|------|-------------|
| **HA / multi-replica** | ~~`StateStore` + `PubSub` traits are already defined in `crates/foster-server/src/store.rs`. Implement `RedisStore` and `RedisPubSub` using `redis-rs`. `AppState` should become generic over both traits. The `version` field on `Snapshot` is the optimistic lock token — use a Redis Lua CAS script or `WATCH`/`MULTI` to reject stale writes.~~ **Done** — `AppState<S, P>` is now generic over `StateStore + Clone` and `PubSub + Clone` with default type params (`InMemoryStore`, `InMemoryPubSub`). `foster_server::router_with(machines, store, pubsub)` accepts any backend; the existing `router()` calls it with the in-memory defaults. `RedisStore` and `RedisPubSub` are implemented in `crates/foster-server/src/store.rs` behind the `redis-backend` feature (enable with `--features redis-backend`). `RedisStore` uses a Lua CAS script for atomic version-checked writes and appends to a Redis list for history (LTRIM capped at 50). `RedisPubSub` spawns one Redis subscriber task per `(session, machine)` channel and fans messages into a local broadcast; `publish()` writes to the Redis channel for cross-replica delivery. |
...
| **Multiple machines per page** | ~~Instance addressing syntax: `fx-machine="counter#1"`. The `#fragment` becomes the instance key appended to the session. WASM client needs to split on `#` when building the session key.~~ **Done** — `split_instance()` in the WASM client splits on `#`. The fragment is appended to the base session with a `.` separator (`{session}.{fragment}`), making `effective_session` unique per instance. Context cache key is `"{machine_id}:{effective_session}"` so two `counter#1` / `counter#2` roots on the same page each maintain independent diff chains. Dev overlay panel IDs also incorporate the effective session; panel ID is stamped as `data-fx-panel` on the root element so `update_overlay` routes to the correct panel. |
| **Generated TypeScript SDK** | ~~Derive typed `sendEvent(event: CounterEvent, payload?: ...) → Promise<Snapshot>` and `setState(snap: Snapshot)` from the machine definition. Output alongside the Playwright spec in `gen_tests`.~~ **Done** — `foster_testgen::generate_sdk(machine, base_url)` emits `{name}.sdk.ts` alongside each spec. Exports `{Name}State`, `{Name}Event` union types, a `{Name}Snapshot` interface, and a `{Name}Client` class with `sendEvent`, `getState`, and `setState`. Uses `@msgpack/msgpack` (added to all examples' `devDependencies`) for the `/state` and `/transition` wire protocol. |
| **Compiled machine validation** | ~~Proc-macro that turns a `MachineBuilder` into a compile-time-checked type graph: Rust enum for states, exhaustive `match` on events. Catches unreachable states and missing transitions at compile time.~~ **Done** — `machine_graph!` in `foster-macros` (re-exported from `foster-core`) accepts a declarative `{id, initial, states, transitions}` block. At compile time it validates: initial state exists, every transition source/target is declared, every state is reachable from `initial` (BFS). Emits `compile_error!` on violation. On success generates `{PascalId}State` and `{PascalId}Event` enums, each with `fn as_str(self) -> &'static str` via exhaustive match. Used in the counter example. |
